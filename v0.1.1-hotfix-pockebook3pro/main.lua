...
function ReaderStatistics:initData()
    ...
    -- ==========================================
    -- yanllsama: Fiziksel Sayfa Kilidi (1)
    -- ==========================================
...
local function applyPerfectSync(physical_pages)
        local document = self.document
        local doc_settings = self.ui.doc_settings
        
        -- POCKETBOOK İÇİN GÜVENLİ KONTROL (Çökme engelleyici)
        local pagemap = nil
        if self.view and self.view.pagemap then
            pagemap = self.view.pagemap
        elseif self.ui and self.ui.pagemap then
            pagemap = self.ui.pagemap
        else
            -- Eğer cihaz hazır değilse çökertmek yerine kibarca uyar
            local InfoMessage = require("ui/widget/infomessage")
            local UIManager = require("ui/uimanager")
            UIManager:show(InfoMessage:new{ 
                text = "Cihaz sayfa haritasını henüz yüklemedi.\nBiraz bekleyip tekrar deneyin.", 
                timeout = 4 
            })
            return nil
        end
        ...
        if document and document.buildSyntheticPageMap then
            if document.hasPageMap and document:hasPageMap() then
                cpp = document:getSyntheticPageMapCharsPerPage()
                if cpp <= 0 then cpp = 1500 end
                result_pages = #document:getPageMap()
            else
            ...
            cpp = best_cpp
            if document.getSyntheticPageMapCharsPerPage and document.getSyntheticPageMapCharsPerPage(document) ~= cpp then
                document:buildSyntheticPageMap(cpp)
                result_pages = document.getPageMap and #document:getPageMap() or physical_pages
            end
        end
        ...
        if pagemap then
            ...
            if not pagemap.has_pagemap then
                ...
                if type(pagemap.resetLayout) == "function" then pagemap:resetLayout() end
            end
            if type(pagemap.updateVisibleLabels) == "function" then
                pagemap:updateVisibleLabels()
            end
        end
        
        doc_settings:saveSetting("pagemap_doc_pages", result_pages)
        if self.data then self.data.pages = result_pages end
        
        local total_screens = document and document:getPageCount() or physical_pages
        local calculated_ratio = math.max(1, math.ceil(total_screens / physical_pages))
        ...
        sub_item_table = {
            ...
            -- ==========================================
            {
                ...
                sub_item_table = (self.document and self.ui and self.ui.doc_settings) and {
                            ...
                            UIManager:show(SpinWidget:new{
                                ...
                                callback = function(spin)
                                    ...
                                    UIManager:show(ConfirmBox:new{
                                       ...
                                       ok_callback = function()
                                            local calculated_ratio = applyPerfectSync(physical_pages)
                                            ...
                                            UIManager:show(InfoMessage:new{ 
                                                text = string.format(L("Sabit sayfalar kilitlendi!\nOran: %d dokunuşta 1 sayfa.", "Fixed pages locked!\nRatio: 1 page per %d taps."), calculated_ratio), 
                                                timeout = 4 
                                            })
                                        end,
                                        cancel_callback = function()
                                            ...
                                            if self.view and type(self.view.refresh) == "function" then self.view:refresh() end
                                            UIManager:broadcastEvent(Event:new("UsePageLabelsUpdated"))
                                            UIManager:show(InfoMessage:new{ text = string.format(L("Kayıt Başarılı!\nOran: %d dokunuşta 1 sayfa.", "Saved!\nRatio: 1 page per %d taps."), calculated_ratio), timeout = 4 })
                                        end
                                    })
                                end,
                            })
                    ...
                    {
                        text = L("2. İnternetten Çek (Google Books)", "2. Fetch from Web (Google Books)"),
                        callback = function()
                            local InfoMessage = require("ui/widget/infomessage")
                            local SpinWidget = require("ui/widget/spinwidget")
                            local ConfirmBox = require("ui/widget/confirmbox")
                            ...
                            UIManager:tickAfterNext(function()
                                local fetched_pages = nil
                                pcall(function()
                                    local https = require("ssl.https"); local ltn12 = require("ltn12"); local json = require("json")
                                    local function urlencode(s) return string.gsub(s, "([^%w ])", function(c) return string.format("%%%02X", string.byte(c)) end):gsub(" ", "+") end
                                    local url = "https://www.googleapis.com/books/v1/volumes?q=intitle:" .. urlencode(book_title) .. "+inauthor:" .. urlencode(book_author)
                                    local body = {}
                                    local _, code = https.request{url=url, sink=ltn12.sink.table(body), protocol="any"}
                                    if code == 200 then
                                        local data = json.decode(table.concat(body))
                                        if data and data.items and data.items[1] then fetched_pages = tonumber(data.items[1].volumeInfo.pageCount) end
                                    end
                                end)
                                if fetched_pages then
                                    UIManager:show(SpinWidget:new{
                                        value = fetched_pages, ok_text = L("Onayla", "Confirm"),
                                        callback = function(spin)
                                            local physical_pages = spin.value
                                            self.ui.doc_settings:saveSetting("yanllsama_physical_pages", physical_pages)
                                            
                                            UIManager:show(ConfirmBox:new{
                                                text = L(string.format("Sabit sayfa numaraları kullanılsın mı?\n\nEvet derseniz, kitabın alt çubuğundaki toplam sayfa sayısı internetten çekilen %d değerine kilitlenecektir.", physical_pages),
                                                         string.format("Use fixed page numbers?\n\nIf Yes, the bottom bar page count will be locked to the fetched %d.", physical_pages)),
                                                ok_text = L("Evet", "Yes"),
                                                cancel_text = L("Hayır", "No"),
                                                ok_callback = function()
                                                    local calculated_ratio = applyPerfectSync(physical_pages)
                                                ...
                                                cancel_callback = function()
                                                    ...
                                                    if self.view and type(self.view.refresh) == "function" then self.view:refresh() end
                                                    UIManager:broadcastEvent(Event:new("UsePageLabelsUpdated"))
                                                    UIManager:show(InfoMessage:new{ text = string.format(L("İnternetten Çekildi!\nOran: %d dokunuşta 1 sayfa.", "Fetched!\nRatio: 1 page per %d taps."), calculated_ratio), timeout = 4 })
                                                end
                                            })
                                        end
                                    })
                                else UIManager:show(InfoMessage:new{ text = L("Bulunamadı.", "Not found."), timeout = 2 }) end
                            end)
                        end,
                    },
                    {
                        ...
                        callback = function()
                            ...
                            local total_screens = self.document:getPageCount() or physical_pages
                            local new_ratio = math.max(1, math.ceil(total_screens / physical_pages))
                            ...
                            UIManager:broadcastEvent(Event:new("UsePageLabelsUpdated"))
                            UIManager:show(InfoMessage:new{ 
                                text = string.format(L("Fonta Göre Ayarlandı!\nOran: %d dokunuşta 1 sayfa.", "Adjusted for Font!\nRatio: 1 page per %d taps."), new_ratio), 
                                timeout = 4 
                            })
                        end,
                    },
                    {
                        text = L("4. Ayarları Sıfırla (Reset)", "4. Reset All Settings"),
                        callback = function()
                            local ConfirmBox = require("ui/widget/confirmbox")
                            local InfoMessage = require("ui/widget/infomessage")
                            local UIManager = require("ui/uimanager")
...
function ReaderStatistics:onPageUpdate(pageno)
    ...
    -- ==========================================
    -- yanllsama: Akıllı Ninja Motoru (Otomatik Oranlı)
    -- ==========================================
    local is_pdf = false
    ...
    if not is_pdf and not closing then
        -- AYAR: Otomatik hesaplanan oranı ayarlardan çek, bulamazsa 1 varsay
        local SCREENS_PER_PAGE = self.ui.doc_settings:readSetting("yanllsama_screen_ratio") or 1 

        self.cre_screen_counter = (self.cre_screen_counter or 0) + 1
        
        if self.cre_screen_counter < SCREENS_PER_PAGE then
            -- BARAJ DOLMADI! Sayfa atlamasını engelle.
            return
        else
            -- BARAJ DOLDU! Sayacı sıfırla ve ilerlemeye izin ver.
            self.cre_screen_counter = 0
        end
    end
    -- ==========================================

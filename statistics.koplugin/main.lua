...
function ReaderStatistics:applyPerfectSync(physical_pages)
    ...
    local pagemap = (self.view and self.view.pagemap) or (self.ui and self.ui.pagemap)
    
    local result_pages = physical_pages
    local best_cpp = 1500
    local cpp = 1500
    
    if document and document.buildSyntheticPageMap then
        -- Higher cpp = more chars per page = fewer pages
        -- So: result_pages < physical_pages means cpp is too high -> lower cpp
        --     result_pages > physical_pages means cpp is too low  -> raise cpp
        local lo, hi = 100, 10000
        local best_diff = math.huge
        
        -- Quick estimate
        document:buildSyntheticPageMap(1500)
        local est = document.getPageMap and #document:getPageMap() or physical_pages
        if est > 0 then
            -- Scale lo/hi around the estimate
            local ratio_guess = math.floor(1500 * est / physical_pages)
            lo = math.max(100, ratio_guess - 2000)
            hi = ratio_guess + 2000
        end
        
        while lo <= hi do
            local mid = math.floor((lo + hi) / 2)
            document:buildSyntheticPageMap(mid)
            result_pages = document.getPageMap and #document:getPageMap() or physical_pages
            ...
            if result_pages == physical_pages then
                break
            elseif result_pages < physical_pages then
                -- cpp too high, decrease it to get more pages
                hi = mid - 1
            else
                -- cpp too low, increase it to get fewer pages
                lo = mid + 1
            end
        end
        -- Ensure best cpp is loaded
        document:buildSyntheticPageMap(cpp)
        result_pages = document.getPageMap and #document:getPageMap() or physical_pages
    end

    -- KOReader Ayarlarina Kaydet
    doc_settings:saveSetting("pagemap_chars_per_synthetic_page", cpp)
    doc_settings:saveSetting("pagemap_use_page_labels", true)
    doc_settings:saveSetting("show_page_overlap_marker", true)
    
    if pagemap then
        pagemap.chars_per_synthetic_page = cpp
        pagemap.use_page_labels = true
        pagemap.page_labels_cache = nil
        -- KRITIK: _postInit applyPerfectSync'den once calisiyor, has_pagemap = false kaliyor.
        -- buildSyntheticPageMap sonrasi manuel olarak set etmek zorundayiz.
        if not pagemap.has_pagemap then
            pagemap.has_pagemap = true
            -- ReaderPageMap'in view modulu kayitli degilse kaydet
            if self.view and self.view.registerViewModule then
                pcall(function()
                    self.view:registerViewModule("pagemap", pagemap)
                end)
            end
        end
    end
    
    doc_settings:saveSetting("pagemap_doc_pages", physical_pages)
    if self.data then self.data.pages = physical_pages end
    
    local total_screens = document and document:getPageCount() or physical_pages
    local calculated_ratio = (total_screens / physical_pages)
    calculated_ratio = math.floor(calculated_ratio * 100 + 0.5) / 100
    doc_settings:saveSetting("yanllsama_screen_ratio", calculated_ratio)
    -- Pagemap modu cache'ini sifirla: bir sonraki onPageUpdate'te
    -- getCurrentPageLabel() ile dogru baslangic saglaniyor.
...
function ReaderStatistics:initData()
    ...
    if physical_pages and summary.status == "complete" then
        if self.ui.paging and self.ui.paging.current_page and self.ui.paging.current_page < self.ui.document:getPageCount() then
    ...
    if physical_pages then
        ...
        _G.yanllsama_current_physical_pages = physical_pages
        if self.document and self.document.getPageCount and physical_pages > 0 then
            self:applyPerfectSync(physical_pages)
        end
    else
    ...
    if not BookStatusWidget._yanllsama_orig_getStatReadPages then
        BookStatusWidget._yanllsama_orig_getStatReadPages = BookStatusWidget.getStatReadPages
        BookStatusWidget.getStatReadPages = function(this)
            local p = this.ui and this.ui.doc_settings and this.ui.doc_settings:readSetting("yanllsama_physical_pages")
            local stats_book = this.ui and this.ui.statistics and this.ui.statistics:getStatsBookStatus() or {}
            if p and stats_book.pages then
                return string.format("%s/%s", stats_book.pages, p)
            end
    ...
    if physical_pages then
        ...
        if not BookMapWidget._yanllsama_orig_update then
            BookMapWidget._yanllsama_orig_update = BookMapWidget.update
            BookMapWidget.update = function(self_bm, ...)
                if self_bm.ui.document then
                    -- BookMapWidget, TOC ve bookmark'lar CRE screen sayfalarini (orn. 1-564) kullanir.
                    -- Eger nb_pages'i fiziki sayfa (384) yaparsak harita kirpilir ve coker.
                    -- Bu yuzden haritanin dogal boyutuyla (564) cizilmesine izin veriyoruz,
                    -- SADECE DB'deki 'read_pages' etiketlerini (orn. 1-384) CRE sayfalarina (1-564) mapliyoruz.
                    
                    local pmap = nil
                    if self_bm.ui.document.getPageMap then
                        pmap = self_bm.ui.document:getPageMap()
                    end
                    local native_count = self_bm.ui.document:getPageCount()
                    
                    local label_to_cre = nil
                    if pmap and #pmap > 0 then
                        label_to_cre = {}
                        for i = 1, #pmap do
                            local lbl = tonumber(pmap[i].label and pmap[i].label:match("%d+"))
                            local cre_page = pmap[i].page  -- 1-564 arasi
                            if lbl and cre_page then
                                if not label_to_cre[lbl] then
                                    label_to_cre[lbl] = {}
                                end
                                table.insert(label_to_cre[lbl], cre_page)
                            end
                        end
                    end

                    local orig_read_pages = self_bm.read_pages
                    if orig_read_pages and label_to_cre then
                        local remapped = {}
                        local max_lbl = pmap and #pmap > 0
                            and tonumber(pmap[#pmap].label and pmap[#pmap].label:match("%d+"))
                            or native_count
                            
                        for lbl, data in pairs(orig_read_pages) do
                            local cre_list = label_to_cre[lbl]
                            if cre_list then
                                -- Etiketin kapsadigi tum CRE sayfalarini (orn. 2 slot) doldur
                                for _, cre_page in ipairs(cre_list) do
                                    remapped[cre_page] = data
                                end
                            elseif max_lbl and max_lbl > 0 then
                                -- Oransal tahmin (fallback)
                                local cre = math.max(1, math.min(native_count,
                                    math.floor(lbl * native_count / max_lbl + 0.5)))
                                remapped[cre] = data
                            else
                                remapped[lbl] = data
                            end
                        end
                        self_bm.read_pages = remapped
                    end

                    local res = self_bm:_yanllsama_orig_update(...)

                    -- Geri yukle
                    if orig_read_pages then self_bm.read_pages = orig_read_pages end
                    return res
                else
                    return self_bm:_yanllsama_orig_update(...)
                end
            end
        end
    end
    ...
    if physical_pages then
        ...
        if db_location then
            ...
            if conn then
                conn:exec(string.format("UPDATE book SET pages = %d WHERE id = %d;", physical_pages, self.id_curr_book))
                conn:close()
            end
        end
    end
...
function ReaderStatistics:resetVolatileStats(now_ts)
    ...
    if now_ts then
        ...
        if self.ui and self.ui.doc_settings then
            ...
            if se == nil then se = true end
            if se and type(self.curr_page) == "number" then
                if self._phys_next then
                    stat_key = self._phys_next
                elseif self._phys_curr then
                    stat_key = self._phys_curr
                else
                    -- Pagemap modu mu?
                    local use_pm = self.ui.pagemap and self.ui.pagemap.has_pagemap
                                   and self.ui.pagemap.use_page_labels
                    if use_pm then
                        local lbl = self.ui.pagemap:getCurrentPageLabel(true)
                        local phys = lbl and tonumber(lbl)
                        if phys then
                            stat_key = phys
                        end
                    else
                        local ratio   = self.ui.doc_settings:readSetting("yanllsama_screen_ratio") or 1.0
                        if self.ui.doc_settings:readSetting("pagemap_chars_per_synthetic_page") then ratio = 1.0 end
                        local max_phys = (self.ui.doc_settings:readSetting("yanllsama_physical_pages"))
                                         or (self.data and self.data.pages) or 9999
                        if ratio < 1.0 then
                            stat_key = math.max(1, math.min(max_phys, math.floor((self.curr_page - 1) / ratio + 1.5)))
                        else
                            stat_key = math.max(1, math.min(max_phys, math.ceil(self.curr_page / ratio)))
                        end
                    end
                end
            end
        end
        self.page_stat[stat_key] = { { now_ts, 0 } }
    end
end
...
function ReaderStatistics:getStatsBookStatus()
    ...
    local p = self.ui and self.ui.doc_settings and self.ui.doc_settings:readSetting("yanllsama_physical_pages")
    if p then
...
function ReaderStatistics:insertDB(updated_pagecount)
    ...
    do
        local ratio = self.ui and self.ui.doc_settings and
            self.ui.doc_settings:readSetting("yanllsama_screen_ratio")
        local se = self.ui and self.ui.doc_settings and
            self.ui.doc_settings:readSetting("yanllsama_sync_enabled")
        if se == nil then se = true end
        if se and type(self.curr_page) == "number" then
            if self._phys_next then
                _stat_curr_key = self._phys_next
            elseif self._phys_curr then
                _stat_curr_key = self._phys_curr
            else
                local use_pm = self.ui.pagemap and self.ui.pagemap.has_pagemap
                               and self.ui.pagemap.use_page_labels
                if use_pm then
                    ...
                    ratio = ratio or 1.0
                    if ratio < 1.0 then
                        _stat_curr_key = math.max(1, math.min(max_phys, math.floor((self.curr_page - 1) / ratio + 1.5)))
                    else
                        _stat_curr_key = math.max(1, math.min(max_phys, math.ceil(self.curr_page / ratio)))
                    end
                end
            end
        end
    end
    local cur_page_data = self.page_stat[_stat_curr_key]
    ...
    stmt:reset():bind(effective_pagecount, id_book):step()
...
function ReaderStatistics:addToMainMenu(menu_items)
    ...
    local function L(tr_text, en_text)
        return is_tr and tr_text or en_text
    end
        ...
        sub_item_table = {
            ...
            -- ==========================================
            {
                ...
                sub_item_table = (self.document and self.ui and self.ui.doc_settings) and {
                    {
                        text = L("Aktif (Açık / Kapalı)", "Active (On / Off)"),
                        checked_func = function()
                            local state = self.ui.doc_settings:readSetting("yanllsama_sync_enabled")
                            if state == nil then return true else return state end
                        end,
                        callback = function()
                            ...
                            local current = self.ui.doc_settings:readSetting("yanllsama_sync_enabled")
                            if current == nil then current = true end
                            local new_state = not current
                            
                            self.ui.doc_settings:saveSetting("yanllsama_sync_enabled", new_state)

                            if not new_state then
                                self.ui.doc_settings:saveSetting("yanllsama_screen_ratio", 1)
                                local pagemap = (self.view and self.view.pagemap) or (self.ui and self.ui.pagemap)
                                if pagemap then
                                    pagemap.use_page_labels = false
                                    pagemap.show_page_labels = false
                                    pagemap.page_labels_cache = nil
                                    pagemap.chars_per_synthetic_page = nil
                                end
                                self.ui.doc_settings:saveSetting("pagemap_use_page_labels", false)
                                self.ui.doc_settings:saveSetting("show_page_overlap_marker", false)
                                self.ui.doc_settings:delSetting("pagemap_chars_per_synthetic_page")
                                self.ui.doc_settings:delSetting("pagemap_doc_pages")
                                
                                if self.document.buildSyntheticPageMap then
                                    self.document:buildSyntheticPageMap(1500)
                                end
                                ...
                                if self.view and type(self.view.refresh) == "function" then self.view:refresh() end
                                UIManager:broadcastEvent(Event:new("UsePageLabelsUpdated"))
                                UIManager:show(InfoMessage:new{ 
                                    text = L("Modül Kapatıldı.\nOrijinal KOReader ayarlarına dönüldü.", "Module Disabled.\nReverted to original settings."), 
                                    timeout = 3 
                                })
                            else
                                UIManager:show(InfoMessage:new{ 
                                    text = L("Modül Açıldı.\nLütfen aşağıdan senkronize edin.", "Module Enabled.\nPlease sync below."), 
                                    timeout = 3 
                                })
                            end
                        end,
                    ...
                    {
                        text = L("1. Manuel Gir", "1. Manual Entry"),
                        callback = function()
                            ...
                            if self.ui.doc_settings:readSetting("yanllsama_sync_enabled") == false then
                                UIManager:show(InfoMessage:new{ text = L("Lütfen önce modülü 'Aktif' konuma getirin.", "Please set the module to 'Active' first."), timeout = 3 })
                                return
                            end
                            
                            local default_val = self.ui.doc_settings:readSetting("yanllsama_physical_pages") or (self.data and self.data.pages) or 1
                            UIManager:show(SpinWidget:new{
                                value = default_val, value_min = 1, value_max = 15000,
                                ok_text = L("Hesapla ve Kaydet", "Calculate & Save"),
                                title_text = L("Fiziksel Sayfa Sayısı", "Physical Page Count"),
                                callback = function(spin)
                                    local physical_pages = spin.value
                                    self.ui.doc_settings:saveSetting("yanllsama_physical_pages", physical_pages)
                                    
                                    UIManager:show(ConfirmBox:new{
                                        text = L(string.format("Sabit sayfa numaraları kullanılsın mı?\n\nEvet derseniz, kitabın alt çubuğundaki ve istatistiklerdeki toplam sayfa sayısı %d olarak kilitlenecektir.", physical_pages),
                                                 string.format("Use fixed page numbers?\n\nIf Yes, the bottom bar and stats page count will be locked to %d.", physical_pages)),
                                        ok_text = L("Evet", "Yes"),
                                        cancel_text = L("Hayır", "No"),
                                       ok_callback = function()
                                            local calculated_ratio = self:applyPerfectSync(physical_pages)
                                            ...
                                            UIManager:show(InfoMessage:new{ 
                                                text = string.format(L("Sabit sayfalar kilitlendi!\nOran: %.2f dokunuşta 1 sayfa.", "Fixed pages locked!\nRatio: %.2f taps per page."), calculated_ratio), 
                                                timeout = 4 
                                            })
                                        end,
                                        cancel_callback = function()
                                            ...
                                            if self.view and type(self.view.refresh) == "function" then self.view:refresh() end
                                            UIManager:broadcastEvent(Event:new("UsePageLabelsUpdated"))
                                            UIManager:show(InfoMessage:new{ text = string.format(L("Kayıt Başarılı!\nOran: %.2f dokunuşta 1 sayfa.", "Saved!\nRatio: %.2f taps per page."), calculated_ratio), timeout = 4 })
                                        end
                                    })
                                end,
                            })
                    ...
                    {
                        text = L("2. İnternetten Çek (Open Library)", "2. Fetch from Web (Open Library)"),
                        callback = function()
                            local InfoMessage = require("ui/widget/infomessage")
                            local SpinWidget = require("ui/widget/spinwidget")
                            local ConfirmBox = require("ui/widget/confirmbox")
                            ...
                            local props = self.document:getProps() or {}
                            local book_title = props.title or ""
                            local book_author = props.authors or ""
                            
                            if book_title == "" then UIManager:show(InfoMessage:new{ text = L("Başlık bulunamadı.", "Title not found."), timeout = 2 }) return end
                            UIManager:show(InfoMessage:new{ text = L("Aranıyor...", "Searching..."), timeout = 2 })
                            UIManager:tickAfterNext(function()
                                local fetched_results = {}
                                ...
                                local ok, err = pcall(function()
                                    ...
                                    local function do_request(url)
                                        ...
                                        if code == 200 then
                                            ...
                                            fetch_error = "HTTP " .. tostring(code)
                                            return nil
                                        end
                                    end

                                    -- === 1. Open Library (rate-limit yok, ücretsiz) ===
                                    local ol_url = "https://openlibrary.org/search.json?title="
                                        .. urlencode(book_title)
                                    if book_author and book_author ~= "" then
                                        ol_url = ol_url .. "&author=" .. urlencode(book_author)
                                    end
                                    ol_url = ol_url .. "&fields=title,author_name,number_of_pages_median&limit=10"
                                    local ol_data = do_request(ol_url)
                                    if ol_data and ol_data.docs then
                                        for _, doc in ipairs(ol_data.docs) do
                                            local pages = tonumber(doc.number_of_pages_median)
                                            local title = doc.title or "?"
                                            local author = (doc.author_name and doc.author_name[1]) or ""
                                            if pages and pages > 10 then
                                                table.insert(fetched_results, { pages = pages, title = title, author = author })
                                            end
                                        end
                                    end

                                    -- === 2. Open Library bulamazsa Google Books (tek istek) ===
                                    if #fetched_results == 0 then
                                        fetch_error = nil  -- reset
                                        local gb_query = urlencode(book_title)
                                        if book_author and book_author ~= "" then
                                            gb_query = gb_query .. "+" .. urlencode(book_author)
                                        end
                                        local gb_url = "https://www.googleapis.com/books/v1/volumes?q="
                                            .. gb_query .. "&maxResults=10"
                                        local gb_data = do_request(gb_url)
                                        if gb_data and gb_data.items then
                                            for i = 1, math.min(10, #gb_data.items) do
                                                local vi = gb_data.items[i].volumeInfo
                                                if vi then
                                                    local pages = tonumber(vi.pageCount) or tonumber(vi.printedPageCount)
                                                    local title = vi.title or "?"
                                                    local author = (vi.authors and vi.authors[1]) or ""
                                                    if pages and pages > 10 then
                                                        table.insert(fetched_results, { pages = pages, title = title, author = author })
                                                    end
                                                end
                                ...
                                if not ok then fetch_error = tostring(err) end

                                local function applyFetchedPages(physical_pages)
                                    self.ui.doc_settings:saveSetting("yanllsama_physical_pages", physical_pages)
                                    UIManager:show(ConfirmBox:new{
                                        text = L(string.format("Sabit sayfa numaraları kullanılsın mı?\n\nEvet derseniz, kitabın alt çubuğundaki toplam sayfa sayısı %d olarak kilitlenecektir.", physical_pages),
                                                 string.format("Use fixed page numbers?\n\nIf Yes, the bottom bar page count will be locked to %d.", physical_pages)),
                                        ok_text = L("Evet", "Yes"), cancel_text = L("Hayır", "No"),
                                        ok_callback = function()
                                            local calculated_ratio = self:applyPerfectSync(physical_pages)
                                            if type(self.resetLayout) == "function" then self:resetLayout() end
                                            if self.view and type(self.view.refresh) == "function" then self.view:refresh() end
                                            UIManager:broadcastEvent(Event:new("UsePageLabelsUpdated"))
                                            UIManager:show(InfoMessage:new{ text = string.format(L("Sabit sayfalar kilitlendi!\nOran: %.2f dokunuşta 1 sayfa.", "Fixed pages locked!\nRatio: %.2f taps per page."), calculated_ratio), timeout = 4 })
                                        end,
                                        cancel_callback = function()
                                            local total_screens = self.document:getPageCount() or physical_pages
                                            -- yanllsama: Kesirli oran (float)
                                            ...
                                            if self.view and type(self.view.refresh) == "function" then self.view:refresh() end
                                            UIManager:broadcastEvent(Event:new("UsePageLabelsUpdated"))
                                            UIManager:show(InfoMessage:new{ text = string.format(L("İnternetten Çekildi!\nOran: %.2f dokunuşta 1 sayfa.", "Fetched!\nRatio: %.2f taps per page."), calculated_ratio), timeout = 4 })
                                        end
                                    })
                                end

                                if #fetched_results > 0 then
                                    if #fetched_results == 1 then
                                        -- Tek sonuç varsa direkt SpinWidget göster
                                        UIManager:show(SpinWidget:new{
                                            value = fetched_results[1].pages, value_min = 1, value_max = 15000,
                                            ok_always_enabled = true,
                                            ok_text = L("Onayla", "Confirm"),
                                            title_text = L("Sayfa Sayısı Doğrula", "Confirm Page Count"),
                                            callback = function(spin) applyFetchedPages(spin.value) end
                                        })
                                    else
                                        -- Birden fazla sonuç: kullanıcıya seçtir
                                        local ButtonDialog = require("ui/widget/buttondialog")
                                        local buttons = {}
                                        for _, result in ipairs(fetched_results) do
                                            local label = string.format("%d syf — %s", result.pages, result.title)
                                            if result.author ~= "" then label = label .. " / " .. result.author end
                                            table.insert(buttons, {{ text = label, callback = function()
                                                UIManager:close(buttons._dialog)
                                                UIManager:show(SpinWidget:new{
                                                    value = result.pages, value_min = 1, value_max = 15000,
                                                    ok_always_enabled = true,
                                                    ok_text = L("Onayla", "Confirm"),
                                                    title_text = L("Sayfa Sayısı Doğrula", "Confirm Page Count"),
                                                    callback = function(spin) applyFetchedPages(spin.value) end
                                        ...
                                        local dialog = ButtonDialog:new{
                                            title = L("Baskıyı Seç", "Select Edition"),
                                            buttons = buttons,
                                        }
                                        buttons._dialog = dialog
                                        UIManager:show(dialog)
                                    end
                                else
                                    local msg
                                    if fetch_error then
                                        msg = L("Hata: ", "Error: ") .. fetch_error
                                    else
                                        msg = L("Bulunamadı: ", "Not found: ") .. book_title
                                    end
                    ...
                    {
                        text = L("3. Font İçin Senkronize Et", "3. Sync for Font Size"),
                        callback = function()
                            local InfoMessage = require("ui/widget/infomessage")
                            local Event = require("ui/event")
                            local UIManager = require("ui/uimanager")
                            ...
                            if not physical_pages then
                                UIManager:show(InfoMessage:new{ text = L("Önce sayfa sayısı girin.", "Enter page count first."), timeout = 3 })
                                return
                            end
                            ...
                            self.ui.doc_settings:saveSetting("yanllsama_screen_ratio", new_ratio)
                            ...
                            UIManager:broadcastEvent(Event:new("UsePageLabelsUpdated"))
                            UIManager:show(InfoMessage:new{ 
                                text = string.format(L("Fonta Göre Ayarlandı!\nOran: %.2f dokunuşta 1 sayfa.", "Adjusted for Font!\nRatio: %.2f taps per page."), new_ratio), 
                                timeout = 4 
                            })
                        end,
                    },
                    {
                        text = L("4. Manuel Oran Gir", "4. Enter Manual Ratio"),
                        callback = function()
                            local SpinWidget = require("ui/widget/spinwidget")
                            local InfoMessage = require("ui/widget/infomessage")
                            local UIManager = require("ui/uimanager")
                            ...
                            local current_ratio = self.ui.doc_settings:readSetting("yanllsama_screen_ratio") or 1.0
                            UIManager:show(SpinWidget:new{
                                value = math.floor(current_ratio * 100), value_min = 1, value_max = 1000, value_step = 1,
                                ok_text = L("Kaydet", "Save"),
                                title_text = L("Oran x100 (Örn: 0.56 için 56)", "Ratio x100 (e.g. 56 for 0.56)"),
                                callback = function(spin)
                                    local new_ratio = spin.value / 100.0
                                    self.ui.doc_settings:saveSetting("yanllsama_screen_ratio", new_ratio)
                                    ...
                                    UIManager:show(InfoMessage:new{ text = string.format(L("Özel Oran Kaydedildi!\nYeni Oran: %.2f", "Custom Ratio Saved!\nNew Ratio: %.2f"), new_ratio), timeout = 4 })
                                end,
                            })
                        end,
                    },
                    {
                        text = L("5. Ayarları Sıfırla (Reset)", "5. Reset All Settings"),
                        callback = function()
                            local ConfirmBox = require("ui/widget/confirmbox")
                            local InfoMessage = require("ui/widget/infomessage")
                            local UIManager = require("ui/uimanager")
                            ...
                            UIManager:show(ConfirmBox:new{
                                text = L("Bu kitap için yaptığınız tüm fiziksel sayfa ayarları silinecek ve orijinal KOReader varsayılanlarına dönülecek.\n\nEmin misiniz?", 
                                         "All physical page settings for this book will be cleared and KOReader defaults restored.\n\nAre you sure?"),
                                ok_text = L("Evet, Sıfırla", "Yes, Reset"),
                                cancel_text = L("İptal", "Cancel"),
                                ok_callback = function()
                                    self.ui.doc_settings:saveSetting("yanllsama_physical_pages", nil)
                                    self.ui.doc_settings:saveSetting("yanllsama_screen_ratio", 1)
                                    
                                    self.ui.doc_settings:saveSetting("pagemap_use_page_labels", false)
                                    self.ui.doc_settings:saveSetting("show_page_overlap_marker", false)
                                    self.ui.doc_settings:saveSetting("pagemap_chars_per_synthetic_page", nil)
                                    self.ui.doc_settings:saveSetting("pagemap_doc_pages", nil)
                                    ...
                                    if self.data and self.document then
                                        ...
                                        if self.id_curr_book and db_location then
                                            local conn = SQ3.open(db_location)
                                            if conn then
                                                conn:exec(string.format("UPDATE book SET pages = %d WHERE id = %d;", self.data.pages, self.id_curr_book))
                                                conn:close()
                                            end
                                        end
                                        -- ==========================================
                                    ...
                                    UIManager:show(InfoMessage:new{ text = L("Her şey sıfırlandı!", "Everything reset!"), timeout = 3 })
                                end
                            })
                        end,
                    },
                    ...
                    {
                        text = L("⚠️ Sadece kitap okurken kullanılabilir.", "⚠️ Only available while reading a book."),
            ...
            {
                text = L("📊 Tekil Kitap İstatistikleri", "📊 Single Book Statistics"),
                sub_item_table = {
                    {
                        text = L("Veri Kaynağı", "Data Source"),
                        sub_item_table = {
                            {
                                text = L("Geçmiş", "History"),
                                checked_func = function()
                                    return (G_reader_settings:readSetting("yanllsama_cbs_data_source") or "history") == "history"
                                end,
                                callback = function()
                                    G_reader_settings:saveSetting("yanllsama_cbs_data_source", "history")
                                end,
                                radio = true,
                            },
                            {
                                text = L("Favoriler", "Favorites"),
                                checked_func = function()
                                    return G_reader_settings:readSetting("yanllsama_cbs_data_source") == "favorites"
                                end,
                                callback = function()
                                    G_reader_settings:saveSetting("yanllsama_cbs_data_source", "favorites")
                                end,
                                radio = true,
                            },
                        }
...
function ReaderStatistics:getCurrentStat()
    ...
    if physical_pages then
        self.data.pages = physical_pages
        if total_read_pages > physical_pages then total_read_pages = physical_pages end
    else
        self.data.pages = self.document:getPageCount()
    end
    ...
    elseif self.document:hasHiddenFlows() then
        ...
        if physical_pages then
            ...
            if self.ui.doc_settings:readSetting("pagemap_chars_per_synthetic_page") then ratio = 1.0 end
            current_page = math.max(1, math.ceil(current_page / ratio))
            if current_page > physical_pages then current_page = physical_pages end
        end
...
function ReaderStatistics:getBookStat(id_book)
    ...
    if self.id_curr_book and id_book == self.id_curr_book and self.ui and self.ui.doc_settings then
        physical_pages = self.ui.doc_settings:readSetting("yanllsama_physical_pages")
        ratio = self.ui.doc_settings:readSetting("yanllsama_screen_ratio") or 1
        if self.ui.doc_settings:readSetting("pagemap_chars_per_synthetic_page") then ratio = 1.0 end
    end
...
function ReaderStatistics:onPageUpdate(pageno)
    ...
    local use_pagemap_mode = false
    if not is_pdf then
        local se = self.ui.doc_settings:readSetting("yanllsama_sync_enabled")
        if se == nil then se = true end
        sync_enabled = se
        if sync_enabled then
            SCREENS_PER_PAGE = self.ui.doc_settings:readSetting("yanllsama_screen_ratio") or 1.0
            if self.ui.doc_settings:readSetting("pagemap_chars_per_synthetic_page") then SCREENS_PER_PAGE = 1.0 end
            physical_pages_locked = self.ui.doc_settings:readSetting("yanllsama_physical_pages")
            -- Pagemap aktifse (sentetik veya gerçek), pagemap modunda çalış
            if self.ui.pagemap and self.ui.pagemap.has_pagemap and self.ui.pagemap.use_page_labels then
                use_pagemap_mode = true
            end
        end
    end

    -- ---- Ratio modu için CRE->fiziksel dönüşüm yardımcısı (pagemap yoksa) ----
    local function cre_to_phys_ratio(cre_pageno, max_phys)
        if SCREENS_PER_PAGE < 1.0 then
            return math.max(1, math.min(max_phys, math.floor((cre_pageno - 1) / SCREENS_PER_PAGE + 1.5)))
        else
            return math.max(1, math.min(max_phys, math.ceil(cre_pageno / SCREENS_PER_PAGE)))
        end
    end

    -- ---- Ara Ekran Zamanı Biriktirme Yardımcısı ----
    local function accumulate_screen_time(phys_page)
        local now_ts = os.time()
        local page_data = self.page_stat[phys_page]
        ...
        if data_tuple then
            ...
            if diff_time >= self.settings.min_sec and diff_time <= self.settings.max_sec then
                valid_time = diff_time
            elseif diff_time > self.settings.max_sec then
                valid_time = self.settings.max_sec
            end
            if valid_time > 0 then
                if data_tuple[2] == 0 then
                    self.mem_read_pages = self.mem_read_pages + 1
                end
                data_tuple[2] = data_tuple[2] + valid_time
                self.mem_read_time = self.mem_read_time + valid_time
            end
            data_tuple[1] = now_ts
        end
    end

    -- ---- Ninja Motor v4: Pagemap modu ----
    -- getCurrentPageLabel() → KOReader'ın kullanıcıya GÖSTERDIĞI sayfa numarası.
    -- onPageUpdate(pageno) çağrısı sırasında getCurrentPageLabel() YENİ sayfayı döndürür.
    -- Önceki fiziksel sayfayı bulmak için _yanllsama_phys_label_prev cache'ini kullan.
    if not is_pdf and not closing and sync_enabled and use_pagemap_mode then
        if type(self.curr_page) == "number" and self.curr_page > 0 then
            -- Yeni sayfanın fiziksel label'ını oku
            local lbl_now = nil
            local phys_now = nil
            if self.ui.pagemap then
                lbl_now = self.ui.pagemap:getCurrentPageLabel(true)
                phys_now = lbl_now and tonumber(lbl_now)
            end
            -- Önceki sayfanın fiziksel label'ı (cache'den)
            local phys_prev = self._yanllsama_phys_label_prev

            if phys_now and phys_prev then
                if phys_now == phys_prev then
                    -- Aynı fiziksel sayfadayız: sadece CRE ekranı değişti. Zamanı biriktir.
                    accumulate_screen_time(phys_prev)
                    self.curr_page = pageno
                    -- Cache'i güncelle (aynı değer ama yine de set et)
                    self._yanllsama_phys_label_prev = phys_now
                    return
                end
                -- Fiziksel sayfa değişti: kayıt için işaretle
                self._phys_curr = phys_prev
                self._phys_next = phys_now
            elseif phys_now and not phys_prev then
                -- İlk geçiş: cache henüz yok, bu sayfa için timestamp aç
                -- (then_ts bloğuna düşecek ve orası halledecek)
                self._yanllsama_phys_label_prev = phys_now
            end
        end
    -- ---- Ninja Motor v4: Ratio modu (pagemap yoksa) ----
    elseif not is_pdf and not closing and sync_enabled and not use_pagemap_mode then
        if type(self.curr_page) == "number" and self.curr_page > 0 then
            local max_phys_b = physical_pages_locked or self.data.pages or 9999
            if SCREENS_PER_PAGE >= 1.0 then
                -- Baraj modu: global ratio ile fiziksel sayfa hesapla
                local phys_prev = cre_to_phys_ratio(self.curr_page, max_phys_b)
                local phys_now  = cre_to_phys_ratio(pageno, max_phys_b)
                if phys_now == phys_prev then
                    -- Aynı fiziksel sayfadayız: sadece CRE ekranı değişti. Zamanı biriktir.
                    accumulate_screen_time(phys_prev)
                    self.curr_page = pageno
                ...
                -- Çarpan modu (ratio < 1): her ekranda birden fazla fiziksel sayfa
                local max_phys = max_phys_b
                local target_phys = cre_to_phys_ratio(pageno, max_phys)
                if not self._phys_curr_anchored then
                    self._phys_curr_anchored = cre_to_phys_ratio(self.curr_page, max_phys)
                end
                local curr_phys = self._phys_curr_anchored
                local next_phys = (target_phys > curr_phys) and target_phys or (curr_phys + 1)
                self._phys_curr_anchored = target_phys
                self._screen_pageno = pageno
                self._phys_curr     = curr_phys
                self._phys_next     = next_phys
            end
        end
    end
    ...
    if sync_enabled then
        if self._phys_curr then
            stat_curr_page = self._phys_curr
            stat_next_page = self._phys_next
        else
            -- _phys_curr set edilmemiş (ilk sayfa, closing, veya pagemap modu ilk geçiş)
            if use_pagemap_mode and self.ui.pagemap then
                -- Pagemap modunda: label'dan fiziksel sayfa al
                local lbl_fb = self.ui.pagemap:getCurrentPageLabel(true)
                local phys_fb = lbl_fb and tonumber(lbl_fb)
                if phys_fb then
                    stat_curr_page = phys_fb
                    stat_next_page = phys_fb
                end
            else
                stat_curr_page = cre_to_phys_ratio(self.curr_page, max_phys_fb)
                stat_next_page = cre_to_phys_ratio(pageno, max_phys_fb)
            end
        end
    end
    ...
    if not then_ts then
        ...
        self._baraj_page_ts = nil
        self._baraj_phys_page = nil
        self._phys_curr_anchored = nil
        self._phys_page_acc = nil
        return
    end
    ...
    if valid_time > 0 then
        -- Süreyi fiziksel sayfa aralığına dağıt
        local start_p, end_p
        if sync_enabled and self._phys_curr then
            start_p = self._phys_curr
            -- Tüm modlarda (Pagemap, Çarpan, Baraj) ileri doğru atlama varsa aradaki sayfaları da kaydet
            if self._phys_next and self._phys_next > self._phys_curr then
                end_p = self._phys_next - 1
            else
                end_p = self._phys_curr
            end
        else
            start_p = stat_curr_page
            end_p   = stat_curr_page
        end
        local pages_in_range = end_p - start_p + 1
        local time_per_page  = math.floor(valid_time / pages_in_range)
        local remainder      = valid_time - time_per_page * pages_in_range

        for p = start_p, end_p do
            local p_data = self.page_stat[p]
            if not p_data then
                p_data = { { then_ts, 0 } }
                self.page_stat[p] = p_data
            end
            local d_tuple = p_data[#p_data]
            local c_dur   = d_tuple[2]
            local allocated = time_per_page + (p == start_p and remainder or 0)
            if allocated > 0 then
                self.mem_read_time = self.mem_read_time + allocated
                if #p_data == 1 and c_dur == 0 then
                    self.mem_read_pages = self.mem_read_pages + 1
                end
                d_tuple[2] = c_dur + allocated
            end
        end
    end
    ...
    -- Yeni sayfa için timestamp girişi
    local new_page_data = self.page_stat[stat_next_page]
    ...
    if self:isEnabledAndNotFrozen() then
        if self._reading_paused_ts then
            ...
            if self.ui and self.ui.doc_settings then
                ...
                if se and type(self.curr_page) == "number" then
                    if self._phys_curr then
                        _pause_stat_key = self._phys_curr
                    else
                        local max_phys = (self.ui.doc_settings:readSetting("yanllsama_physical_pages"))
                                         or (self.data and self.data.pages) or 9999
                        ratio = ratio or 1.0
                        if ratio < 1.0 then
                            _pause_stat_key = math.max(1, math.min(max_phys, math.floor((self.curr_page - 1) / ratio + 1.5)))
                        else
                            _pause_stat_key = math.max(1, math.min(max_phys, math.ceil(self.curr_page / ratio)))
                        end
                    end
                end
            end
            local page_data = self.page_stat[_pause_stat_key]
            local data_tuple = page_data and page_data[#page_data]
            if data_tuple then
